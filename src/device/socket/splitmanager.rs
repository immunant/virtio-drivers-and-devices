...
/// [`SplitConnectionManagerTx::execute`].
#[derive(Clone, Debug)]
pub enum TxAction {
...
impl ConnectionTable {
    ...
    pub fn find(
        ...
        self.connections.iter().find(|c| {
            c.dst == peer && c.src_port == local_port && !c.is_closed()
        })
    }
...
pub struct SplitConnectionManagerTx<H: Hal, T: Transport> {
...
impl<H: Hal, T: Transport> SplitConnectionManagerTx<H, T> {
    ...
    pub fn connections_mut(&mut self) -> &mut ConnectionTable {
    ...
    pub fn execute(&mut self, action: TxAction) -> Result {
        match action {
    ...
    SplitConnectionManagerRx<H, T, RX_BUFFER_SIZE>
{
    ...
    /// Pass any returned `TxAction` to [`SplitConnectionManagerTx::execute`].
    ///
    /// The `connections` argument must be the same `ConnectionTable` held by the TX side.
    /// In practice, the caller coordinates access (e.g. the TX side calls this on behalf
    /// of the RX side, or the connection table is behind a lock for the brief duration
    /// of poll).
    pub fn poll(
        &mut self,
        ...
        let result = self.rx.poll(|event, body| {
            ...
            // Determine any TX action needed
...
pub struct SplitDeviceConnectionManagerTx<H: DeviceHal, T: DeviceTransport> {
...
impl<H: DeviceHal, T: DeviceTransport> SplitDeviceConnectionManagerTx<H, T> {
    /// Returns a reference to the connection table.
    pub fn connections(&self) -> &ConnectionTable {
        &self.connections
    }
