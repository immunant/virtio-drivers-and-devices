...
impl<H: DeviceHal, T: DeviceTransport, L: LockFactory> VsockManager for VsockDeviceConnectionManager<H, T, L> {
    fn accept(&self, c: Connection) -> Result {
        Self::accept(self, c)
    }
...
impl<M: VirtIOSocketManager<L::Lock<Connection>>, L: LockFactory> VsockConnectionManagerCommon<M, L> {
    /// Allows incoming connections on the given port number.
    pub fn listen(&self, port: u32) {
    ...
    pub fn poll_direct(&self) -> Result<Option<VsockEvent>> {
        ...
        self.driver.poll(|mut event, body| {
            ...
            if event.event_type == VsockEventType::ConnectionRequest {
                ...
                // reset package before the connection is accepted, it will be treated as an
                // unknown connection.
                return Ok(Some(event));
            }
            let connection = get_connection_for_event::<L>(&self.inner.lock().connections, &event, local_cid);
            let Some(connection) = connection else {
                return Ok(None);
            };
            let mut connection = connection.1;
            connection.info.update_for_event(&event);
    ...
    pub fn recv(&self, peer: VsockAddr, src_port: u32, buffer: &mut [u8]) -> Result<usize> {
        let mut inner = self.inner.lock();
        let (connection_index, mut connection) = get_connection::<L>(&inner.connections, peer, src_port)?;
